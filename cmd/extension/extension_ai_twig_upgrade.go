...
import (
	...
	"github.com/onlishop/onlishop-cli/internal/llm"
	"github.com/onlishop/onlishop-cli/internal/twigparser"
	"github.com/onlishop/onlishop-cli/internal/verifier"
...
var extensionAiTwigUpgradeCmd = &cobra.Command{
	Use:   "twig-upgrade [path] [old-onlishop-version] [new-onlishop-version]",
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		for _, sourceDirectory := range toolCfg.SourceDirectories {
			...
			oldVersion, err := cloneOnlishopStorefront(cmd.Context(), args[1])
			if err != nil {
				return err
			}
			...
			newVersion, err := cloneOnlishopStorefront(cmd.Context(), args[2])
			if err != nil {
				return err
			}
...
func cloneOnlishopStorefront(ctx context.Context, version string) (string, error) {
	tempDir, err := os.MkdirTemp(os.TempDir(), "onlishop")
	if err != nil {
		return "", err
	}

	git := exec.CommandContext(ctx, "git", "-c", "advice.detachedHead=false", "clone", "-q", "--branch", "v"+version, "https://github.com/onlishop/storefront", tempDir, "--depth", "1")
