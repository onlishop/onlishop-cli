...
var extensionValidateCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		// If the user does not want to run full validation, only run onlishop-cli
		if !isFull {
			only = "sw-cli"
		}
		...
		if storeCompliance || os.Getenv("ONLISHOP_CLI_STORE_COMPLIANCE") == "1" {
...
func init() {
	...
	extensionValidateCmd.PersistentFlags().String("check-against", "highest", "Check against Onlishop Version (highest, lowest)")
