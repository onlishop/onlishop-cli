...
var projectAutofixComposerCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err := huh.NewInput().
			Title("Please enter the Onlishop Packagist Token").
			Value(&token).
			Run(); err != nil {
			return err
		}
		...
		for _, extension := range extensions {
			...
			composerInstall = append(composerInstall, fmt.Sprintf("store.onlishop.com/%s:%s", strings.ToLower(extName), extVersion.String()))
		...
		if !rootComposerJson.Repositories.HasRepository("https://packages.onlishop.com") {
		...
		auth.BearerAuth["packages.onlishop.com"] = token
