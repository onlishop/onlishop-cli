...
var projectWorkerCmd = &cobra.Command{
	...
	RunE: func(cobraCmd *cobra.Command, args []string) error {
		...
		if projectRoot, err = findClosestOnlishopProject(); err != nil {
			return err
		}
		...
		if queuesToConsume == "" {
			if is, _ := shop.IsOnlishopVersion(projectRoot, ">=6.5.7"); is {
				consumeArgs = append(consumeArgs, "async", "failed", "low_priority")
			} else if is, _ := shop.IsOnlishopVersion(projectRoot, ">=6.5"); is {
		...
		baseName := fmt.Sprintf("onlishop-cli-%d", os.Getpid())
