...
func findClosestOnlishopProject() (string, error) {
	...
	for {
		...
		for _, file := range files {
			if _, err := os.Stat(file); err == nil {
				...
				if strings.Contains(contentString, "onlishop/core") {
	...
	return "", fmt.Errorf("cannot find Onlishop project in current directory")
}
