...
var accountCompanyProducerExtensionUploadCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		constraint, err := zipExt.GetOnlishopVersionConstraint()
		if err != nil {
			logging.FromContext(cmd.Context()).Debugf("Failed to get Onlishop version constraint: %v", err)
			return err
		}

		logging.FromContext(cmd.Context()).Debugf("Onlishop version constraint: %s", constraint)
