...
func PrepareFolderForZipping(ctx context.Context, path string, ext Extension, extCfg *Config) error {
	...
	minOnlishopVersionConstraint, err := ext.GetOnlishopVersionConstraint()
	if err != nil {
		return fmt.Errorf(errorFormat, err)
	}

	minVersion, err := lookupForMinMatchingVersion(ctx, minOnlishopVersionConstraint)
	if err != nil {
		return fmt.Errorf("lookup for min matching version: %w", err)
	}

	onlishop65Constraint, _ := version.NewConstraint(">=6.5.0")

	if onlishop65Constraint.Check(version.Must(version.NewVersion(minVersion))) {
		return nil
	}
...
func filterRequires(composer map[string]interface{}, extCfg *Config) map[string]interface{} {
	...
	keys := []string{"onlishop/platform", "onlishop/core", "onlishop/onlishop", "onlishop/storefront", "onlishop/administration", "onlishop/elasticsearch", "composer/installers"}
	if extCfg != nil {
...
func addComposerReplacements(composer map[string]interface{}, minVersion string) (map[string]interface{}, error) {
	...
	for _, component := range components {
		packageName := fmt.Sprintf("onlishop/%s", component)
...
func GetOnlishopVersions(ctx context.Context) ([]string, error) {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, "https://repo.packagist.org/p2/onlishop/core.json", http.NoBody)
	if err != nil {
		return nil, fmt.Errorf("create composer version request: %w", err)
	}

	req.Header.Set("User-Agent", "Onlishop CLI")
...
func lookupForMinMatchingVersion(ctx context.Context, versionConstraint *version.Constraints) (string, error) {
	versions, err := GetOnlishopVersions(ctx)
	if err != nil {
		return "", fmt.Errorf("get onlishop versions: %w", err)
	}
