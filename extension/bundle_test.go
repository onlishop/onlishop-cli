...
func TestCreateBundleEmptyFolder(t *testing.T) {
	...
	bundle, err := newOnlishopBundle(dir)
	assert.Error(t, err)
	assert.Nil(t, bundle)
}
...
func TestCreateBundleInvalidComposerType(t *testing.T) {
	...
	bundle, err := newOnlishopBundle(dir)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "composer.json type is not onlishop-bundle")
	assert.Nil(t, bundle)
}
...
func TestCreateBundleMissingName(t *testing.T) {
	...
	bundle, err := newOnlishopBundle(dir)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "composer.json does not contain onlishop-bundle-name")
	assert.Nil(t, bundle)
}
...
func TestCreateBundle(t *testing.T) {
	...
	composer := []byte(`{
		...
		"type": "onlishop-bundle",
		"extra": {
			"onlishop-bundle-name": "TestBundle"
		},
		"autoload": {
			"psr-4": {
				"TestBundle\\": "src/"
	...
	assert.Equal(t, TypeOnlishopBundle, bundle.GetType())
