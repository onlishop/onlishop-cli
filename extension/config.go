...
import (
	...
	"github.com/onlishop/onlishop-cli/internal/changelog"
...
type ConfigBuild struct {
	...
	// Override the onlishop version constraint for building, can be used to specify the version of the onlishop to use for building
	OnlishopVersionConstraint string `yaml:"onlishopVersionConstraint,omitempty"`
...
type ConfigBuildZipAssets struct {
	// When enabled, the onlishop-cli build the assets
	Enabled bool `yaml:"enabled"`
...
type ConfigStore struct {
	...
	// Specifies whether the extension should automatically be set compatible with Onlishop bugfix versions.
...
func readExtensionConfig(dir string) (*Config, error) {
	...
	config.FileName = ".onlishop-extension.yml"

	configLocation := ""

	if _, err := os.Stat(filepath.Join(dir, ".onlishop-extension.yml")); err == nil {
		configLocation = filepath.Join(dir, ".onlishop-extension.yml")
	} else if _, err := os.Stat(filepath.Join(dir, ".onlishop-extension.yaml")); err == nil {
		configLocation = filepath.Join(dir, ".onlishop-extension.yaml")
	} else {
		return config, nil
	}
