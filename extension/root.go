...
const (
	...
	TypeOnlishopBundle = "onlishop-bundle"

	ComposerTypePlugin = "onlishop-platform-plugin"
	ComposerTypeApp    = "onlishop-app"
	ComposerTypeBundle = "onlishop-bundle"
)

func GetExtensionByFolder(path string) (Extension, error) {
	if _, err := os.Stat(fmt.Sprintf("%s/plugin.xml", path)); err == nil {
		return nil, fmt.Errorf("onlishop 5 is not supported. Please use https://github.com/FriendsOfOnlishop/FroshPluginUploader instead")
	}
	...
	if err != nil {
		if errors.Is(err, ErrPlatformInvalidType) {
			ext, err = newOnlishopBundle(path)
		} else {
			return nil, err
		}
	}
...
type Extension interface {
	...
	GetOnlishopVersionConstraint() (*version.Constraints, error)
	GetType() string
	GetPath() string
