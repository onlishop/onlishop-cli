...
func GetOnlishopProjectConstraint(project string) (*version.Constraints, error) {
	...
	constraint, ok := composer.Require["onlishop/core"]
	...
	if !ok {
		...
		return nil, fmt.Errorf("missing onlishop/core requirement in composer.json")
	}
...
func getProjectConstraintFromKernel(project string) (*version.Constraints, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("could not determine onlishop version")
	}
	...
	for _, pkg := range composer.Packages {
		if pkg.PackageType == ComposerTypePlugin || pkg.PackageType == ComposerTypeBundle || pkg.PackageType == ComposerTypeApp {
			...
			case ComposerTypeBundle:
				ext.(*OnlishopBundle).Composer.Version = pkg.Version
			}
