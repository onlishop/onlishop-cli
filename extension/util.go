...
func PlatformPath(projectRoot, component, path string) string {
	...
	} else if _, err := os.Stat(filepath.Join(projectRoot, "vendor", "onlishop", "platform")); err == nil {
		return filepath.Join(projectRoot, "vendor", "onlishop", "platform", "src", component, path)
	}

	return filepath.Join(projectRoot, "vendor", "onlishop", strings.ToLower(component), path)
}

// projectRequiresBuild checks if the project is a contribution project aka onlishop/onlishop.
func projectRequiresBuild(projectRoot string) bool {
	// We work inside Onlishop itself
	if _, err := os.Stat(filepath.Join(projectRoot, "src", "Core", "composer.json")); err == nil {
		return true
	}

	// vendor/onlishop/platform does never have assets pre-build
	if _, err := os.Stat(filepath.Join(projectRoot, "vendor", "onlishop", "platform", "composer.json")); err == nil {
		return true
	}
