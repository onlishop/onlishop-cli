...
func (a App) GetOnlishopVersionConstraint() (*version.Constraints, error) {
	if a.config != nil && a.config.Build.OnlishopVersionConstraint != "" {
		v, err := version.NewConstraint(a.config.Build.OnlishopVersionConstraint)
		if err != nil {
			return nil, err
		}
...
func (a App) Validate(_ context.Context, check validation.Check) {
	...
	if a.manifest.Setup != nil && a.manifest.Setup.Secret != "" {
		check.AddResult(validation.CheckResult{
			...
			Message:    "The xml element setup:secret is only for local development, please remove it. You can find your generated app secret on your extension detail page in the master data section. For more information see https://docs.onlishop.com/en/onlishop-platform-dev-en/app-system-guide/setup#authorisation",
			Severity:   validation.SeverityError,
		})
	}
}
