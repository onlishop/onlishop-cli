...
import (
	...
	"github.com/onlishop/onlishop-cli/internal/asset"
	"github.com/onlishop/onlishop-cli/internal/ci"
	"github.com/onlishop/onlishop-cli/internal/esbuild"
...
func BuildAssetsForExtensions(ctx context.Context, sources []asset.Source, assetConfig AssetBuildConfig) error { // nolint:gocyclo
	...
	minVersion, err := lookupForMinMatchingVersion(ctx, assetConfig.OnlishopVersion)
	if err != nil {
		return err
	}

	requiresOnlishopSources := cfgs.RequiresOnlishopRepository()

	onlishopRoot := assetConfig.OnlishopRoot
	if onlishopRoot == "" && requiresOnlishopSources {
		onlishopRoot, err = setupOnlishopInTemp(ctx, minVersion)
		if err != nil {
			return err
		}

		defer deletePaths(ctx, onlishopRoot)
	}
	...
	if onlishopRoot != "" && len(assetConfig.KeepNodeModules) > 0 {
		paths = slices.DeleteFunc(paths, func(path string) bool {
			rel, err := filepath.Rel(onlishopRoot, path)
			if err != nil {
				return false
			}
	...
	if !assetConfig.DisableAdminBuild && cfgs.RequiresAdminBuild() {
		...
		if len(nonCompatibleExtensions) != 0 {
			if projectRequiresBuild(onlishopRoot) {
				...
				if onlishopRoot == "" {
					...
					basePath = strings.TrimLeft(
						strings.Replace(PlatformPath(onlishopRoot, "Storefront", ""), onlishopRoot, "", 1),
						"/",
					) + "/"
				}
			...
			if err := prepareOnlishopForAsset(onlishopRoot, nonCompatibleExtensions); err != nil {
				return err
			}

			administrationRoot := PlatformPath(onlishopRoot, "Administration", "Resources/app/administration")
			...
			envList := []string{fmt.Sprintf("PROJECT_ROOT=%s", onlishopRoot), fmt.Sprintf("ADMIN_ROOT=%s", PlatformPath(onlishopRoot, "Administration", ""))}

			if !projectRequiresBuild(onlishopRoot) || !assetConfig.ForceAdminBuild {
				logging.FromContext(ctx).Debugf("Building only administration assets for plugins")
				envList = append(envList, "ONLISHOP_ADMIN_BUILD_ONLY_EXTENSIONS=1", "ONLISHOP_ADMIN_SKIP_SOURCEMAP_GENERATION=1")
			} else {
	...
	if !assetConfig.DisableStorefrontBuild && cfgs.RequiresStorefrontBuild() {
		...
		if len(nonCompatibleExtensions) != 0 {
			...
			storefrontRoot := PlatformPath(onlishopRoot, "Storefront", "Resources/app/storefront")
			...
			envList := []string{
				"NODE_ENV=production",
				fmt.Sprintf("PROJECT_ROOT=%s", onlishopRoot),
...
func prepareOnlishopForAsset(onlishopRoot string, cfgs ExtensionAssetConfig) error {
	varFolder := fmt.Sprintf("%s/var", onlishopRoot)
	if _, err := os.Stat(varFolder); os.IsNotExist(err) {
		err := os.Mkdir(varFolder, os.ModePerm)
		if err != nil {
			return fmt.Errorf("prepareOnlishopForAsset: %w", err)
		}
	}
	...
	err = os.WriteFile(fmt.Sprintf("%s/var/plugins.json", onlishopRoot), pluginJson, os.ModePerm)
	if err != nil {
		return fmt.Errorf("prepareOnlishopForAsset: %w", err)
	}

	err = os.WriteFile(fmt.Sprintf("%s/var/features.json", onlishopRoot), []byte("{}"), os.ModePerm)
	if err != nil {
		return fmt.Errorf("prepareOnlishopForAsset: %w", err)
	}
