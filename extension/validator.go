...
import (
	...
	"github.com/onlishop/onlishop-cli/internal/spdx"
...
func runDefaultValidate(ext Extension, check validation.Check) {
	...
	_, onlishopVersionErr := ext.GetOnlishopVersionConstraint()
	...
	// Skip version validation for OnlishopBundle
	if versionErr != nil && ext.GetType() != TypeOnlishopBundle {
		check.AddResult(validation.CheckResult{
			Path:       rootFile,
	...
	if onlishopVersionErr != nil {
		check.AddResult(validation.CheckResult{
			Path:       rootFile,
			Identifier: "metadata.onlishop_version",
			Message:    onlishopVersionErr.Error(),
			Severity:   validation.SeverityError,
		})
	}
	...
	// Skip description validation for OnlishopBundle
	if ext.GetType() != TypeOnlishopBundle {
		...
		if len([]rune(metaData.Description.Chinese)) < 150 || len([]rune(metaData.Description.Chinese)) > 185 {
			check.AddResult(validation.CheckResult{
				Path:       rootFile,
				Identifier: "metadata.description",
				Message:    fmt.Sprintf("in composer.json, the german description with length of %d should have a length from 150 up to 185 characters.", len([]rune(metaData.Description.Chinese))),
				Severity:   validation.SeverityError,
			})
		}
