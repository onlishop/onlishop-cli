...
- **Build the CLI**: `go build -o onlishop-cli .`
...
### Core Command Groups
1. **`account/`** - Onlishop Account management (login, companies, extensions)
2. **`extension/`** - Extension development tools (build, validate, AI assistance)
3. **`project/`** - Onlishop project management (creation, configuration, deployment)

### Key Internal Packages
- **`internal/verifier/`** - Code quality tools (PHPStan, ESLint, Twig linting)
- **`internal/account-api/`** - Onlishop Account API integration
...
- **Platform Plugins**: Standard Onlishop 6 plugins with `composer.json`
- **Onlishop Apps**: App system extensions with `manifest.xml`
- **Onlishop Bundles**: Custom bundle implementations

Extension detection is automatic based on file presence. Each type has specific build, validation, and packaging rules.

### Configuration Files
- **`.onlishop-cli.yaml`** - Global CLI configuration
- **`.onlishop-extension.yml`** - Extension-specific settings (schema: `extension/onlishop-extension-schema.json`)
- **`.onlishop-project.yml`** - Project-specific settings (schema: `shop/onlishop-project-schema.json`)
...
onlishop-cli extension build

# Watch mode for development
onlishop-cli extension admin-watch

# Validate extension
onlishop-cli extension validate

# Create distribution package
onlishop-cli extension zip
```

### Project Management
```bash
# Create new project
onlishop-cli project create

# Build assets
onlishop-cli project admin-build
onlishop-cli project storefront-build

# Development servers
onlishop-cli project admin-watch
onlishop-cli project storefront-watch
```
