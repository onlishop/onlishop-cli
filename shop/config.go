...
import (
	...
	"github.com/onlishop/onlishop-cli/internal/system"
)

type Config struct {
	AdditionalConfigs []string `yaml:"include,omitempty"`
	// The URL of the Onlishop instance
	URL              string            `yaml:"url"`
...
type ConfigBuild struct {
	...
	// When enabled, the assets of extensions will be removed from the extension public folder. (Requires Onlishop 6.5.2.0)
	...
	// When enabled, the onlishop admin will be built
...
func ReadConfig(fileName string, allowFallback bool) (*Config, error) {
	...
	if os.IsNotExist(err) {
		...
		return nil, fmt.Errorf("cannot find project configuration file \"%s\", use onlishop-cli project config init to create one", fileName)
	}
...
func DefaultConfigFileName() string {
	...
	if err != nil {
		return ".onlishop-project.yml"
	}

	if _, err := os.Stat(path.Join(currentDir, ".onlishop-project.yaml")); err == nil {
		return ".onlishop-project.yaml"
	}
