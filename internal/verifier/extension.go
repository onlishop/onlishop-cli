...
func determineVersionRange(cfg *ToolConfig, versionConstraint *version.Constraints) error {
	versions, err := extension.GetOnlishopVersions(context.Background())
	if err != nil {
		return err
	}
	...
	cfg.MinOnlishopVersion = matchingVersions[0].String()
	cfg.MaxOnlishopVersion = matchingVersions[len(matchingVersions)-1].String()
