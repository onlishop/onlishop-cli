...
func SetupTools(ctx context.Context, currentVersion string) error {
	customToolDir := os.Getenv("ONLISHOP_CLI_TOOLS_DIR")
	if customToolDir != "" {
	...
	cacheDir := system.GetOnlishopCliCacheDir()
	...
	if ok, err := system.IsPHPVersionAtLeast(ctx, "8.2.0"); err != nil {
		...
		return fmt.Errorf("php version must be at least 8.2.0 to use this. Update your PHP version or use the onlishop-cli docker image")
	}
	...
	if ok, err := system.IsNodeVersionAtLeast(ctx, "20.0.0"); err != nil {
		...
		return fmt.Errorf("node.js version must be at least 20.0.0 to use this. Update your Node.js version or use the onlishop-cli docker image")
	}
