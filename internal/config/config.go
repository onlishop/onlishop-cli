...
type configData struct {
	Account struct {
		Email    string `env:"ONLISHOP_CLI_ACCOUNT_EMAIL" yaml:"email"`
		Password string `env:"ONLISHOP_CLI_ACCOUNT_PASSWORD" yaml:"password"`
		Company  int    `env:"ONLISHOP_CLI_ACCOUNT_COMPANY" yaml:"company"`
	} `yaml:"account"`
}
...
func InitConfig(configPath string) error {
	...
	if len(configPath) > 0 {
		...
		state.cfgPath = fmt.Sprintf("%s/.onlishop-cli.yml", configDir)
	}
