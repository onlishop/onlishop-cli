...
func TestMigrateComposerJson(t *testing.T) {
	t.Run("successful migration", func(t *testing.T) {
		...
		initialComposer := &packagist.ComposerJson{
			Name: "onlishop/project",
			Require: packagist.ComposerPackageLink{
				"onlishop/recovery": "1.0.0",
				"php":               "^7.4",
			},
		...
		assert.False(t, migratedComposer.HasPackage("onlishop/recovery"))
		...
		assert.Equal(t, "*", migratedComposer.RequireDev["onlishop/dev-tools"])
		...
		require.True(t, ok)
		assert.Contains(t, endpoints, "https://raw.githubusercontent.com/onlishop/recipes/flex/main/index.json")
