...
func GetFutureExtensionUpdates(ctx context.Context, currentVersion string, futureVersion string, extensions []UpdateCheckExtension) ([]UpdateCheckExtensionCompatibility, error) {
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, "https://api.onlishop.com/swplatform/autoupdate", nil)
	if err != nil {
		return nil, err
	}
	...
	q.Set("onlishopVersion", currentVersion)
	req.URL.RawQuery = q.Encode()

	bodyBytes, err := json.Marshal(map[string]interface{}{
		"futureOnlishopVersion": futureVersion,
		"plugins":               extensions,
	})
	if err != nil {
		return nil, err
