...
var httpUserAgent = "onlishop-cli/0.0.0"
...
func (c *Client) NewAuthenticatedRequest(ctx context.Context, method, path string, body io.Reader) (*http.Request, error) {
	...
	r.Header.Set("x-onlishop-token", c.Token.Token)
...
const CacheFileName = "onlishop-api-client-token.json"
...
func getApiTokenCacheFilePath() (string, error) {
	...
	onlishopCacheDir := filepath.Join(cacheDir, "onlishop-cli")
	return filepath.Join(onlishopCacheDir, CacheFileName), nil
}
