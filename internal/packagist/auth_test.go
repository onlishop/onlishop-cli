...
func TestFillAuthStruct(t *testing.T) {
	...
	t.Run("with ONLISHOP_PACKAGES_TOKEN", func(t *testing.T) {
		t.Setenv("ONLISHOP_PACKAGES_TOKEN", "my-token")
		auth := &ComposerAuth{}
		filledAuth := fillAuthStruct(auth)

		assert.Equal(t, "my-token", filledAuth.BearerAuth["packages.onlishop.com"])
	})
	...
	t.Run("with both env variables", func(t *testing.T) {
		...
		composerAuth := `{
			"bearer": {
				"packages.onlishop.com": "composer-token"
			}
		}`
		t.Setenv("COMPOSER_AUTH", composerAuth)
		auth := &ComposerAuth{}
