...
func (p *PackageResponse) HasPackage(name string) bool {
	expectedName := fmt.Sprintf("store.onlishop.com/%s", strings.ToLower(name))
...
func GetPackages(ctx context.Context, token string) (*PackageResponse, error) {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, "https://packages.onlishop.com/packages.json", nil)
	if err != nil {
		return nil, err
	}
